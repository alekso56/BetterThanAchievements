...
public class AchCard extends DriverItem {
	...
	public class Environment extends li.cil.oc.api.prefab.ManagedEnvironment {
		public Environment(EnvironmentHost container) {
			this.setNode(Network.newNode(this, Visibility.Neighbors).withComponent("ach").create());
		}

		// goal1: have achievements that can be unlocked by computers
		// .unlockAchievement("playername","achievementname")
		...
		// goal2: be able to check achievements from computer
		// .checkAchievement("playername","achievementname")
		@Callback(direct = true, limit = 1)
		public Object[] checkAchievement(Context context, Arguments args) {
			...
			if (achievement != null) {
				EntityPlayerMP player = getPlayer(playername);
				if (player != null) {
					Object[] result = new Object[] { player.getStatFile().hasAchievementUnlocked(achievement),
